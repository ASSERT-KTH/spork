...
public class Compare {
    ...
    /**
     * Compare two unnamed modules for equality, without regard to the order of unordered elements.
     * This method only considers the import statements attached to the module by Spork, and the
     * packages (recursively starting with the root packages). It does not consider other module
     * level information, as Spork does not deal with such.
     *
    ...
    /**
     * Compare the left package with the right package, without regard to the order of unordered
     * type members.
     *
     * @param left A CtPackage.
     * @param right A CtPackage.`
     * @return true iff the two packages are equal, without regard to the order of unordered
     *     elements.
     */
    private static boolean compare(CtPackage left, CtPackage right) {
        if (!left.getQualifiedName().equals(right.getQualifiedName())) return false;
        ...
        boolean subPacksMatch =
                IntStream.range(0, sortedLeftSubpackages.size())
                        .mapToObj(
                                i ->
                                        compare(
                                                sortedLeftSubpackages.get(i),
                                                sortedRightSubpackages.get(i)))
                        .allMatch(b -> b);
     ...
     *
     * <p>1. Nested types are recursively sorted, then ordered by simple names and placed last. 2.
     * Methods are ordered by signature and placed above nested classes. 3. Constructors are also
     * ordered by signature and placed above methods. 3. All other type members retain their
     * original order with respect to each other, and are placed at the top.
     *
     * <p>
     *
