...
def create_merge_dirs(
    merge_dir_base: pathlib.Path, file_merges: List[Tuple[git.Commit, List[Mapping[gitutils.Revision, git.Blob]]]]
) -> List[pathlib.Path]:
    ...
    for merge_commit, merge_revisions_list in file_merges:
        for merge_revisions in merge_revisions_list:
            ...
            _write_blob_to_file(merge_dir / "Expected.java", result_blob)
            _write_blob_to_file(merge_dir / "Left.java", left_blob)
            _write_blob_to_file(merge_dir / "Right.java", right_blob)
            if gitutils.Revision.BASE in merge_revisions:
                _write_blob_to_file(merge_dir / "Base.java", merge_revisions[gitutils.Revision.BASE])
            else:
                # If left and right added the same file, there will be no base blob
                LOGGER.warning(f"No base blob found for merge commit {merge_commit.hexsha} and result blob {result_blob.hexsha}")
